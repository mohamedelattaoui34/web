...
<body>
    <div class="down-tabs">
        <div id="con1" class="dTab">
                                 ...
                                 <?php
                                                $select_formation = $conn->prepare("SELECT * FROM `groups`  WHERE étudiant_id = ?");
                                                $select_formation->execute([$étudiant_id]);
        ...
        <?php
                                        $select_groups = $conn->prepare("SELECT DISTINCT Enseignants_id FROM `groups` WHERE étudiant_id = ?");
                                        $select_groups->execute([$étudiant_id]);
                                            if($select_groups->rowCount() > 0){
                                            while($fetch_groups = $select_groups->fetch(PDO::FETCH_ASSOC)){
                                                $Enseignants_id = $fetch_groups['Enseignants_id'];
                                                ...
                                                $select_Enseignants->execute([$Enseignants_id]);
                                                    ...
                                                    $fetch_Enseignants = $select_Enseignants->fetch(PDO::FETCH_ASSOC)
                                            ?>
                                            <a href="ms_por_ense.php?Enseignants_id=<?= $Enseignants_id; ?>">                                                 
                                            <?php
        ...
        <?php
                                            ...
                                            ?>
                                            <a href="ms_por_admin.php?admin_id=<?= $admin_id; ?>">
    ...
    <div class="up-tabs">
        ...
        <div id="two" class="tab"><i class="fa-solid fa-user-tie"></i>Les Enseignants</div>

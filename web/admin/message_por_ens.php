...
if (isset($_POST['publish'])) {
    ...
    $insert_post = $conn->prepare("INSERT INTO `message_admin_enseig` (admin_id, content, Enseignants_id) VALUES (?,?,?)");
...
WHERE message_admin_enseig.admin_id = ? AND message_admin_enseig.Enseignants_id = ?
...
WHERE message_Enseignants_admin.admin_id = ? AND message_Enseignants_admin.Enseignants_id = ?
...
$select_messages->execute([$admin_id, $get_id, $admin_id, $get_id]);
        ...
        if ($select_messages->rowCount() > 0) {
            ...
            echo "<div class='message-container admin-message'>  </div>";
        }
        ?>
    </div>

...
if(isset($_POST['delete_formation'])){
    $delete_formation_id = $_POST['formation_id'];
    $delete_formation_id = filter_var($delete_formation_id, FILTER_SANITIZE_STRING);
    $delete_formation = $conn->prepare("DELETE FROM `formation` WHERE id = ?");
      ...
      <div class="main">
        ...
        <div class="table-cont">
          ...
          <table>
            ...
            <?php
                  $select_formation = $conn->prepare("SELECT * FROM `formation` ORDER BY id DESC");
                  $select_formation->execute();
                  if($select_formation->rowCount() > 0){
                    while($fetch_formation = $select_formation->fetch(PDO::FETCH_ASSOC)){ 
                        $formation = $fetch_formation['id'];
                  ?>
            <tbody>
              <tr>

                <td class="tden">
                <?= $fetch_formation['title']; ?>
                </td>

                <td>                
                    <a href="view_groop_lang.php?numbre=<?= $fetch_formation['title']; ?>"><?= $fetch_formation['title']; ?></a>
                </td>

                <td>
                    <?= $fetch_formation['status']; ?>
                        ...
                        <td>
                                ...
                                $date_fr = date("j ", strtotime($fetch_formation['date'])) . $mois[date("n", strtotime($fetch_formation['date'])) - 1] . date(" Y", strtotime($fetch_formation['date']));
                                echo $date_fr;
                                ?>                                
                        </td>
                        <td class="tden">
                            <a href="demanders.php?formation=<?= $formation; ?>"><i class="fa-solid fa-plus"></i></a>
                        </td>

                        <td>
                            <a href="view_groups.php?formation=<?= $formation; ?>"><i class="fa-solid fa-eye"></i></a>
                        </td>


                        <td>
                            <a href="Modifier.php?id=<?= $formation; ?>"><i class="fa-solid fa-pen"></i></a>
                        </td>
                        ...
                        <td>
                            ...
                            <button type="submit" class="inline-delete-btn" name="delete_formation" onclick="return confirm('Voulez-vous vraiment supprimer ?');" style="background: white;

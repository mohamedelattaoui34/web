...
if(isset($_POST['delete_Enseignants'])){
    $delete_Enseignants_id = $_POST['Enseignants_id'];
    $delete_Enseignants_id = filter_var($delete_Enseignants_id, FILTER_SANITIZE_STRING);
    $delete_Enseignants = $conn->prepare("DELETE FROM `Enseignants` WHERE id = ?");
    $delete_Enseignants->execute([$delete_Enseignants_id]);
      ...
      <div class="main">
        ...
        <div class="table-cont">
          ...
          <table>
            ...
            <?php
                  $select_Enseignants = $conn->prepare("SELECT * FROM `Enseignants` ORDER BY id DESC");
                  ...
                  if($select_Enseignants->rowCount() > 0){
                    while($fetch_Enseignants = $select_Enseignants->fetch(PDO::FETCH_ASSOC)){ 
                        $Enseignants_id = $fetch_Enseignants['id'];
                  ?>
            <tbody>
              <tr>
                ...
                <?php
                    ...
                    ?>
                    <img src="../uploaded_img/<?= $fetch_Enseignants['image']; ?>" style="width:34px; height:34px; border-radius:50%;" alt="" aria-hidden="true">
                    <?php 
                }else {
            ?>      
                    <img src="../img/chico.jpg" alt="" style="width:34px; height:34px; border-radius:50%;">   
            ...
            ?>
                ...
                <td>                
                 <?= $fetch_Enseignants['Numéro']; ?>
                        ...
                        <td>
                            <a href="view_groups.php?Enseignants_id=<?= $Enseignants_id; ?>"><i class="fa-solid fa-eye"></i></a>
                        </td>
                        ...
                        <td>
                            ...
                            <input type="hidden" name="Enseignants_id" value="<?= $fetch_Enseignants['id']; ?>">
                            <button type="submit" class="inline-delete-btn" name="delete_Enseignants" onclick="return confirm('Voulez-vous vraiment supprimer ?');" style="background: white;

...
if(isset($_POST['delete_étudiant'])){
    ...
    $delete_étudiant = $conn->prepare("DELETE FROM `demanders` WHERE id = ?");
...
if(isset($_POST['ajoute'])){
  ...
  $étudiant_id = $_POST['étudiant_id'];
  $étudiant_id = filter_var($étudiant_id, FILTER_SANITIZE_STRING);
  $Enseignants_id = $_POST['Enseignants_id'];
  $Enseignants_id = filter_var($Enseignants_id, FILTER_SANITIZE_STRING);


  $check_query = $conn->prepare("SELECT COUNT(*) FROM groups WHERE étudiant_id = ? AND formation_id = ?");
  $check_query->execute([$étudiant_id, $formation_id]);
  $result = $check_query->fetchColumn();

  if($result == 0) {
 
     $insert_post = $conn->prepare("INSERT INTO `groups`(étudiant_id, formation_id, Enseignants_id, Acceptation) VALUES(?,?,?,1)");
...
<style>
  ...
  <body>
    ...
    <div class="right">
      ...
      <div class="main">
        <h2 class="ti">Tableau de  demande</h2>
        ...
        <div class="table-cont">
          ...
          <table>
            ...
            <?php
                  $select_demanders = $conn->prepare("SELECT * FROM `demanders` WHERE formation_id = ? ORDER BY id DESC");
                  $select_demanders->execute([$get_id]);
                  if($select_demanders->rowCount() > 0){
                    while($fetch_demanders = $select_demanders->fetch(PDO::FETCH_ASSOC)){ 
            ...
            <tbody>
              <tr>
                ...
                <?php
                  ...
                  $select_étudiant->execute([$fetch_demanders['étudiant_id']]);
                  if($select_étudiant->rowCount() > 0){
                   $fetch_étudiant = $select_étudiant->fetch(PDO::FETCH_ASSOC)

                  ?>
                  <img src="../uploaded_img/<?= $fetch_étudiant['image']; ?>" alt="" style="width:34px; height:34;">
                ...
                <td>
                            <?php
                                ...
                                $date_fr = date("j ", strtotime($fetch_demanders['date'])) . $mois[date("n", strtotime($fetch_demanders['date'])) - 1] . date(" Y", strtotime($fetch_demanders['date']));
                                echo $date_fr;
                                ?>                                
                        </td>
                        ...
                        <form action="" method="POST">
                              <input type="hidden" name="formation_id" value="<?= $fetch_demanders['formation_id']; ?>">
                              <input type="hidden" name="Enseignants_id" value="<?= $fetch_demanders['Enseignants_id']; ?>">
                              <input type="hidden" name="étudiant_id" value="<?= $fetch_demanders['étudiant_id']; ?>">
                              <button type="submit" class="inline-ajoute" name="ajoute">ajoute</button>
                            </form>                       
                        </td>

                        <td>
                            <form action="" method="POST">
                            <input type="hidden" name="étudiant_id" value="<?= $fetch_demanders['id']; ?>">
                            <button type="submit" class="inline-delete-btn" name="delete_étudiant" onclick="return confirm('هل تريد حذف الطلب');"><i class="fa-solid fa-trash"></i></button>
                            </form>
                        </td>

...
if (isset($_POST['publish'])) {
    ...
    $insert_post = $conn->prepare("INSERT INTO `message_ense_env` (étudiant_id, content, Enseignants_id) VALUES (?,?,?)");
    $insert_post->execute([$get_id, $content, $Enseignants_id]);
}
...
$select_messages = $conn->prepare("SELECT 'teacher' as source, 
       message_ense_env.id, 
       message_ense_env.content, 
       Enseignants.name as sender_name, 
       Enseignants.image as sender_image, 
       message_ense_env.date as created_at 
FROM message_ense_env
INNER JOIN Enseignants ON message_ense_env.Enseignants_id = Enseignants.id 
WHERE message_ense_env.Enseignants_id = ? AND message_ense_env.étudiant_id = ?

UNION ALL

SELECT 'student' as source, 
message_étudiant_env.id, 
message_étudiant_env.content, 
étudiant.name as sender_name, 
étudiant.image as sender_image, 
message_étudiant_env.date as created_at 
FROM message_étudiant_env
INNER JOIN étudiant ON message_étudiant_env.étudiant_id = étudiant.id 
WHERE message_étudiant_env.Enseignants_id = ? AND message_étudiant_env.étudiant_id = ?
...
$select_messages->execute([$Enseignants_id, $get_id, $Enseignants_id, $get_id]);
    ...
    <section class="post-editor" dir="rtl">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <textarea name="content" class="box" required maxlength="10000" placeholder=" entrer un message..."></textarea>

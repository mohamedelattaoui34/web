...
if (isset($_POST['submit'])) {
  ...
  $cours_id = filter_var($cours_id, FILTER_SANITIZE_STRING);
  
  $check_duplicate = $conn->prepare("SELECT COUNT(*) FROM `cours` WHERE cours_id = ?");
  $check_duplicate->execute([$get_id]);
 ...
 $insert_user = $conn->prepare("INSERT INTO `cours`(title, code, cours_id) VALUES(?,?,?)");
 $insert_user->execute([$title, $code, $cours_id]);
  }
  ...
  <head>
    ...
    <style>
      input.btn {
        ...
        input[type="password"] {
        border: 1px solid rgb(148 150 152);
        padding: 5px;
        border-radius: 7px;
        width: 100px;
      }
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>Code du cours</th>
                    <th>Ajouter des leçons</th>
                </tr>
            </thead>

            <?php
            
            $select_cours = $conn->prepare("SELECT * FROM `message_ense_env_groups` WHERE Enseignants_id = ? AND id = ?");
            $select_cours->execute([$Enseignants_id, $get_id]);
            if($select_cours->rowCount() > 0){
                while($fetch_cours = $select_cours->fetch(PDO::FETCH_ASSOC)){
                    $cours_id = $fetch_cours['id'];
            ?>
            <tbody>
                <tr>
                    <td>
                        
                        <p><?= $fetch_cours['title_cours']; ?></p>
                    </td>
                    <td>
                        
                        <form action="" method="post">
                            <input type="hidden" name="cours_id" value="<?= $cours_id; ?>">
                            <input type="hidden" name="title" value="<?= $fetch_cours['title_cours']; ?>">
                            <div class="koo">
                                <input type="password" name="code" required placeholder="Mot de passe" />
                                <input type="submit" class="btn" name="submit" value="Ajouter le code" />
                            </div>
                        </form>
                    </td>
                    <td>
                        <a href="add_cours.php?cours_id=<?= $cours_id; ?>"><i class="fa-solid fa-plus"></i></a>
                    </td>
                </tr>
            </tbody>
            <?php

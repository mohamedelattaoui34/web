...
if (isset($_POST['submit'])) {
   ...
   $image_folder = '../uploaded_img/' . $image;
      ...
      $check_std_id = $conn->prepare("SELECT * FROM Enseignants WHERE email = ?");
   ...
   $insert_Enseignants = $conn->prepare("INSERT INTO `Enseignants`(name, email, password, Numéro, image) VALUES(?,?,?,?,?)");
   $insert_Enseignants->execute([$name, $email, $pass, $Numéro, $image]);
   $select_Enseignants = $conn->prepare("SELECT * FROM `Enseignants` WHERE email = ? AND password = ?");
   $select_Enseignants->execute([$email, $pass]);
   $row = $select_Enseignants->fetch(PDO::FETCH_ASSOC);
   if($select_Enseignants->rowCount() > 0){
      $_SESSION['Enseignants_id'] = $row['id'];
      header('location:Enseignants.php');
        ...
        <?php
         ...
         if(isset($_POST['submuit'])){
           ...
           if($select_Enseignants->rowCount() > 0){
              ...
              $_SESSION['Enseignants_id'] = $fetch_Enseignants_id['id'];
              header('location:Enseignants.php');
           }else{

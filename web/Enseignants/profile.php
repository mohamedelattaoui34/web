...
<body>
    ...
    <div class="Container">
        <div class="backCover">
            <a href="Enseignants.php"><i class="fa-solid fa-arrow-right"></i></a>
                ...
                <i onclick="ouv();" id="bell" class="fa-solid fa-bell">
                    ...
                    <div class="ko">
                        ...
                        <?php
                            ...
                            $select_profile = $conn->prepare("SELECT * FROM `demanders` WHERE Enseignants_id = ?");
                            $select_profile->execute([$Enseignants_id]);
                            if($select_profile->rowCount() > 0){
                            while($fetch_profile = $select_profile->fetch(PDO::FETCH_ASSOC)){
                                $étudiant_id = $fetch_profile['étudiant_id'];
                                $formation = $fetch_profile['formation_id'];
                            ...
                            $select_formation->execute([$formation]);
                                ...
                                $formation_cl = $conn->prepare("SELECT * FROM `groups` WHERE formation_id = ? AND étudiant_id = ?");
                                $formation_cl->execute([$formation, $étudiant_id]);
            ?>
            <a href="demanders.php?formation=<?= $fetch_profile['formation_id']; ?>">
            <p style="<?php if($formation_cl->rowCount() > 0){ echo 'background-color: white;'; } ?>" class="dema">
            Envoyer <?= $fetch_étudiant['name']; ?>Demande d'adhésion <?= $fetch_formation['title']; ?> <span class="daat"><?= $fetch_profile['date']?></span>
            </p>
            </a>
            <?php }}}}  ?>
                ...
                <div class="conImg">
                <a href="edit_photo.php?Enseignants_id=<?= $Enseignants_id; ?>"><i class="fa-solid fa-camera"></i></a>

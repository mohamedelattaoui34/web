...
if (isset($_POST['submit'])) {
   ...
   $check_std_id = $conn->prepare("SELECT * FROM étudiant WHERE email = ?");
   $check_std_id->execute([$email]);

   if ($check_std_id->rowCount() > 0) {
   // E-mail en double trouvé
       $message[] = "   E-mail en double  ";
   } else {


   $insert_user = $conn->prepare("INSERT INTO `étudiant`(name, email, password, Numéro, image) VALUES(?,?,?,?,?)");
   $insert_user->execute([$name, $email, $pass, $Numéro, $image]);
   $select_user = $conn->prepare("SELECT * FROM `étudiant` WHERE email = ? AND password = ?");
   $select_user->execute([$email, $pass]);
   $row = $select_user->fetch(PDO::FETCH_ASSOC);
   if($select_user->rowCount() > 0){
      $_SESSION['étudiant_id'] = $row['id'];
      header('location:index.php');
    ...
    <div class="container">
      <div class="forms-container">
        ...
        <div id="loginImg" class="logoBackground">
          <img class="login"  src="./admin/login.jpg" alt="">
        </div>
        ...
        <?php
          ...
          if(isset($_POST['submuit'])){
            ...
            $select_Enseignants = $conn->prepare("SELECT * FROM `étudiant` WHERE email = ? AND password = ?");
            $select_Enseignants->execute([$email, $pass]);
            
            if($select_Enseignants->rowCount() > 0){
               $fetch_Enseignants_id = $select_Enseignants->fetch(PDO::FETCH_ASSOC);
               $_SESSION['étudiant_id'] = $fetch_Enseignants_id['id'];
               header('location:index.php');
            }else{
